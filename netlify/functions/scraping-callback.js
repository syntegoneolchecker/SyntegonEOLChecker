...
async function retryBlobsOperation(operationName, operation, maxRetries = 5) {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			const isJobNotFound =
				error.message?.includes("Job") && error.message?.includes("not found");
...
async function triggerNextPendingUrl(job, jobId, baseUrl) {
	...
	logger.debug(
		`[CALLBACK] Found pending URL ${nextUrl.index}: ${nextUrl.url}. Triggering fetch.`
	);
...
exports.handler = async function (event, context) {
	...
	try {
		...
		try {
			logger.debug(
				`[CALLBACK] Attempting to save URL result for job ${jobId}, URL ${urlIndex}`
			);
		...
		if (allDone) {
			logger.debug(
				`[CALLBACK] ✓ All URLs complete for job ${jobId}. Polling loop will trigger analysis.`
			);
		} else {
