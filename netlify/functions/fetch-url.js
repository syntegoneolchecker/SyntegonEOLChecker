...
async function scrapeNBKProductWithBrowserQL(productUrl) {
	...
	logger.info(
		`NBK BrowserQL: Successfully scraped product page (${result.content.length} characters)`
	);
...
const fetchUrlHandler = async function (event, context) {
	...
	try {
		...
		logger.debug(
			`[FETCH-URL] Fetching URL ${urlIndex} for job ${jobId}: ${url} (method: ${scrapingMethod || "render"})`
		);
		logger.debug(
			`[FETCH-URL] Extracted values: jpUrl=${jpUrl}, usUrl=${usUrl}, model=${model}, fallbackUrl=${fallbackUrl}`
		);
...
async function handleCommonError(params) {
	...
	const errorMessage =
		error.message.includes("503") || error.message.includes("Service restarting")
...
async function continuePipelineAfterError(params) {
	...
	if (allDone) {
		...
		if (job && job.status !== "analyzing" && job.status !== "complete") {
			...
			logger.info(
				`All URLs complete but analysis already started (status: ${job?.status}), skipping duplicate trigger`
			);
		}
	} else {
		const job = await getJob(jobId, context);
...
async function handleNbkInteractive(params) {
	...
	try {
		...
		logger.info(
			`NBK: Successfully scraped product page (${productResult.content.length} characters)`
		);
...
async function handleNbkNoResults(params) {
	const { jobId, urlIndex, url, snippet, model, preprocessedModel, baseUrl, context } = params;
...
async function handleBrowserQL(params) {
	...
	try {
		...
		logger.debug(
			`[BROWSERQL] BrowserQL scraping complete for URL ${urlIndex}. All done: ${allDone}`
		);
...
async function continuePipelineAfterSuccess(params) {
	...
	logger.debug(
		`[PIPELINE] continuePipelineAfterSuccess called for job ${jobId}, allDone=${allDone}`
	);
	...
	if (allDone) {
		...
		if (job) {
			logger.debug(
				`[PIPELINE] Job retrieved, checking for pending URLs. Total URLs: ${job.urls?.length}`
			);
			const nextUrl = job.urls.find((u) => u.status === "pending");
			if (nextUrl) {
				logger.debug(
					`[PIPELINE] Found pending URL ${nextUrl.index}: ${nextUrl.url}, triggering fetch`
				);
				await triggerFetch(jobId, nextUrl, baseUrl);
...
function handleRenderServiceResult(serviceResult, methodPrefix) {
	...
	if (!serviceResult.success) {
		throw new Error(
			`Render invocation failed: ${serviceResult.error?.message || "Unknown error"}`
		);
	}
...
async function handleRenderDefault(params) {
	...
	logger.info(
		`Calling Render scraping service for URL ${urlIndex}: ${scrapingServiceUrl}/scrape`
	);
