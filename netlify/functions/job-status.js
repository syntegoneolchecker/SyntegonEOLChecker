...
const jobStatusHandler = async function (event, context) {
	...
	try {
		...
		logger.debug(
			`[STATUS] Job retrieved: status=${job.status}, urls=${job.urls?.length}, completed=${job.urls?.filter((u) => u.status === "complete").length}`
		);
		...
		logger.debug(
			`[STATUS] Returning status for job ${jobId}: ${job.status}, URLs: [${job.urls?.map(formatUrlStatus).join(", ")}]`
		);
