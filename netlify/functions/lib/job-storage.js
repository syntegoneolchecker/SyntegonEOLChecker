		...
		if (shouldDeleteJob(job)) {
			...
			try {
				...
				if (deleteError.statusCode === 404 || deleteError.message?.includes("404")) {
					logger.info(
						`Job ${blob.key} was already deleted by another process (race condition handled)`
					);
					return false;
				}
...
function handleBlobError(error, blobKey) {
	...
	if (statusCode === 403 || errorMessage.includes("403")) {
		logger.warn(
			`⚠️  Skipping blob ${blobKey}: Permission denied (403). This blob may be orphaned from an older version.`
		);
...
async function saveJobUrls(jobId, urls, _context) {
	...
	await store.setJSON(jobId, job);
	logger.info(
		`Updated job ${jobId} status: ${previousStatus} → urls_ready (${urls.length} URLs added)`
	);
}
...
async function updateJobStatus(jobId, status, error, _context, metadata = {}) {
	...
	if (!job) {
		throw new Error(
			`Job ${jobId} not found. Jobs are automatically deleted ${config.JOB_CLEANUP_DELAY_MINUTES} minutes after completion.`
		);
	}
...
async function saveUrlResult(jobId, urlIndex, result, _context) {
	...
	logger.info(
		`[STORAGE DEBUG] Job retrieved. Current URL statuses: [${job.urls?.map(formatUrlStatus).join(", ")}]`
	);
	...
	if (url) {
		...
		url.status = "complete";
		logger.info(
			`[STORAGE DEBUG] URL ${urlIndex} status changed: ${previousStatus} -> complete`
		);
	} else {
	...
	await store.setJSON(jobId, job);
	logger.info(
		`[STORAGE DEBUG] Job saved successfully. URL statuses after save: [${job.urls?.map(formatUrlStatus).join(", ")}]`
	);

	// Check if all URLs are complete
	const allComplete = job.urls.every((u) => u.status === "complete");
	logger.info(
		`[STORAGE DEBUG] All URLs complete check: ${allComplete} (${job.urls.filter((u) => u.status === "complete").length}/${job.urls.length})`
	);
	return allComplete;
}
