...
function determineTablesToKeep(tables, adjacentThreshold) {
	...
	tables.forEach((table, i) => {
		...
		if (
			i < tables.length - 1 &&
			isAdjacentToProductTable(tables, i + 1, i, adjacentThreshold)
		) {
			tablesToKeep.add(i + 1);
...
function filterIrrelevantTables(content, productModel) {
	...
	logger.debug(
		`filterIrrelevantTables: kept ${tablesToKeep.size}/${allTables.length} tables, removed ${tablesToRemove.length}`
	);
...
function smartTruncate(content, maxLength, productModel) {
	...
	processedContent = removeDistantContent(
		processedContent,
		productModel,
		maxLength - TRUNCATION_MSG.length
	);
	...
	if (processedContent.length + TRUNCATION_MSG.length > maxLength) {
		logger.warn(
			`Content still over limit after all stages (${processedContent.length + TRUNCATION_MSG.length} > ${maxLength}), applying hard truncation`
		);
...
function removeBoilerplate(content) {
	...
	const boilerplatePatterns = [
		...
		new RE2(
			String.raw`^(Home|About|Contact|Products|Services|Support|FAQ|Login|Register|Cart|Checkout)[\s|]*$`,
			"gim"
		),
		// Copyright notices
...
function calculateZoneRadius(positionCount, maxLength) {
	...
	return {
		radius: Math.max(
			config.ZONE_RADIUS_MIN,
			Math.min(config.ZONE_RADIUS_MAX, Math.floor(idealZoneSize / 2))
		),
...
function removeDistantContent(content, productModel, maxLength) {
	...
	logger.info(
		`Found ${importantPositions.length} important positions (${productPositions.length} product mentions, ${keywordPositions.length} keywords)`
	);

	// Calculate adaptive zone radius
	const { radius: ZONE_RADIUS, estimatedZones } = calculateZoneRadius(
		importantPositions.length,
		maxLength
	);
	logger.info(
		`Using adaptive zone radius: ${ZONE_RADIUS} chars (estimated ${estimatedZones} zones after merging)`
	);
