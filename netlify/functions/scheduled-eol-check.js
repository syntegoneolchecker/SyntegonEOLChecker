...
function getCurrentDeploymentUrl(event, context) {
	...
	if (context?.clientContext?.custom?.netlify) {
		try {
			const decoded = Buffer.from(context.clientContext.custom.netlify, "base64").toString(
				"utf-8"
			);
...
const handler = async (event, context) => {
	...
	try {
		...
		if (state.lastResetDate !== currentDate) {
			logger.info(
				`New day detected (${currentDate}), resetting counter from ${state.dailyCounter} to 0`
			);
			// Use set-auto-check-state to avoid race condition
			const resetResponse = await fetch(
				`${siteUrl}/.netlify/functions/set-auto-check-state`,
				{
					method: "POST",
		...
		if (serpapiResponse.ok) {
			...
			if (serpapiData.remaining <= config.MIN_SERPAPI_CREDITS_FOR_AUTO) {
				logger.info(
					`SerpAPI credits too low (${serpapiData.remaining}), disabling auto-check`
				);
				// Use set-auto-check-state to avoid race condition
		...
		logger.info(
			`Environment: DEPLOY_PRIME_URL=${process.env.DEPLOY_PRIME_URL}, DEPLOY_URL=${process.env.DEPLOY_URL}, URL=${process.env.URL}`
		);
