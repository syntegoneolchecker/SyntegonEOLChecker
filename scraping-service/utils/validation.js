...
function isSafePublicUrl(url) {
	try {
		...
		if (hostname.startsWith("169.254.")) {
			...
			return {
				valid: false,
				reason: "Cannot scrape link-local addresses (cloud metadata blocked)"
			};
		}

		// SSRF Protection: Block other reserved/special IP ranges
		if (
			hostname.startsWith("0.") || // 0.0.0.0/8 (current network)
	...
	try {
		...
		const isAllowed = allowedOrigins.some((origin) => {
			try {
				...
				return (
					callbackHostname === allowedHostname ||
		...
		if (!isAllowed) {
			return {
				valid: false,
				reason: `Callback URL domain not in allowed list. Allowed origins: ${allowedOrigins.join(", ")}`
			};
		}
