...
function cleanJavaScriptPatterns(text) {
	...
	const patterns = {
		...
		i18nEntry: new RE2(
			String.raw`'[\w.-]+(?:\.[\w.-]+)*'\s*:\s*[\x60'"][^\x60'"]*[\x60'"],?\s*`,
			"g"
		),
	...
	try {
		...
		try {
			...
			if (fullText.length > 0) {
				logger.info(
					`✓ pdf-parse extracted ${fullText.length} chars from PDF (${Math.min(5, data.numpages)} pages)`
				);
				return fullText;
			}
...
async function tryFastFetch(url, timeout = 5000) {
	...
	if (!urlValidation.valid) {
		logger.error(
			`SSRF protection: Blocked unsafe URL in tryFastFetch: ${url} - ${urlValidation.reason}`
		);
		return null;
	}
