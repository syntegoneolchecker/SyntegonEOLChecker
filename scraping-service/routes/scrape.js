    ...
    return enqueuePuppeteerTask(async () => {
        ...
        try {
            ...
            if (!urlValidation.valid) {
                console.error(`SSRF protection: Blocked unsafe URL for Puppeteer: ${url} - ${urlValidation.reason}`);
                throw new Error(`Invalid URL for scraping: ${urlValidation.reason}`);
            }

            // Navigate with timeout handling
            const navigationResult = await navigateWithTimeout(page, url, waitStrategy, navTimeout, pendingRequests);
            ...
            // Extract content
            const { content, pageTitle } = await extractContentSafely(page, navigationTimedOut, url);
            ...
            console.error(`[${new Date().toISOString()}] Puppeteer error:`, puppeteerError.message);
 ...
 */
async function navigateWithTimeout(page, url, waitStrategy, navTimeout, pendingRequests) {
    ...
    try {
        ...
        if (navError.message.includes('timeout') || navError.message.includes('Navigation timeout')) {
            console.log(`Navigation timed out after ${navTimeout/1000}s, continuing with extraction`);
 ...
 * @param {boolean} isCloudflareProtected - Whether site is Cloudflare-protected
...
async function extractContentSafely(page, navigationTimedOut, url) {
    try {
        ...
        if (navigationTimedOut) {
            ...
            console.log(`Content length: ${result.content.length} characters (extracted after timeout)`);
        } else {
...
function validateAndFixContent(content) {
    if (!content || content.length < 50) {
        console.warn(`⚠️  Empty or invalid content (${content ? content.length : 0} chars), adding explanation`);
        return `[The website could not be scraped - extracted only ${content ? content.length : 0} characters. The site may require authentication, use anti-bot protection, or be temporarily unavailable.]`;
    }
    return content;
}
...
async function handleScrapeRequest(req, res) {
    ...
    try {
        ...
        if (fastResult) {
            return await handleFastFetchSuccess(fastResult, url, callbackUrl, { jobId, urlIndex, snippet }, res);
        }

        // Check if it's a PDF or text file - don't use Puppeteer
        if (isPDFUrl(url) || isTextFileUrl(url)) {
            return await handlePDFOrTextFileFailed(url, callbackUrl, { jobId, urlIndex, snippet }, res);
        }

        // Use Puppeteer for dynamic HTML pages
        console.log(`Fast fetch failed, using Puppeteer for ${url}...`);
        const result = await handlePuppeteerScraping(url, callbackUrl, { jobId, urlIndex, snippet, url }, res);
 ...
 */
async function handleFastFetchSuccess(fastResult, url, callbackUrl, callbackData, res) {
