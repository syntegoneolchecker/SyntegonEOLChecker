...
async function handlePuppeteerScraping(url, callbackUrl, callbackData, _res) {
	return enqueuePuppeteerTask(async () => {
		...
		try {
			...
			logger.error(`[${new Date().toISOString()}] Puppeteer error:`, puppeteerError.message);
...
async function navigateWithTimeout(page, url, waitStrategy, navTimeout, pendingRequests) {
	...
	try {
		...
		if (
			...
			logger.info(
				`Navigation timed out after ${navTimeout / 1000}s, continuing with extraction`
			);
...
async function handleScrapeRequest(req, res) {
	...
	logger.info(
		`[${new Date().toISOString()}] Request #${requestCount} - Memory: ${memBefore.rss}MB RSS`
	);
	...
	try {
		...
		logger.error(`[${new Date().toISOString()}] Scraping error:`, error.message);
