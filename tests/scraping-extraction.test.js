...
jest.mock("../scraping-service/utils/encoding", () => ({
	decodeWithProperEncoding: jest.fn((buffer) => buffer.toString("utf8"))
}));
...
const {
	...
	extractHTMLText,
	isErrorPage
} = require("../scraping-service/utils/extraction");
...
describe("Scraping Service - Extraction Utilities", () => {
	...
	describe("isErrorPage", () => {
		it("should detect 404 Not Found pages", () => {
			expect(isErrorPage("404 Not Found - The page you are looking for does not exist")).toBe(
				true
			);
		});

		it("should detect 500 Internal Server Error pages", () => {
			expect(isErrorPage("500 Internal Server Error - Something went wrong on our end")).toBe(
				true
			);
		});
		...
		it("should detect 403 Forbidden pages", () => {
			expect(
				isErrorPage("403 Forbidden - You do not have permission to access this resource")
			).toBe(true);
		});
		...
		it('should detect "Access Denied" text', () => {
			expect(isErrorPage("Access Denied - You are not authorized to view this content")).toBe(
				true
			);
		});
		...
		it("should detect Japanese error pages", () => {
			expect(
				isErrorPage("ページが見つかりませんでした。お探しのページは存在しません。")
			).toBe(true);
		});
		...
		it("should detect another Japanese error pattern", () => {
			expect(
				isErrorPage(
					"申し訳ございませんが、ご指定のページが見つかりませんでした。別のページをお試しください。"
				)
			).toBe(true);
		});
		...
		it("should return false for normal content", () => {
			const normalContent =
				"This is a product specification page for the XYZ-100 sensor. " +
		...
		it("should return false for content with product information", () => {
			const content =
				"Model: ABC-123 | Status: Available | Price: $199.99 | " +
	...
	describe("extractHTMLText", () => {
		...
		it("should remove nav elements", () => {
			const html =
				'<nav><a href="/">Home</a><a href="/about">About</a></nav><p>Main content</p>';
			const result = extractHTMLText(html);
		...
		it("should preserve table structure with pipe delimiters", () => {
			const html =
				"<table><tr><th>Model</th><th>Status</th></tr><tr><td>ABC-123</td><td>Active</td></tr></table>";
			const result = extractHTMLText(html);
