...
describe("Scraping Extraction - Extended Tests", () => {
	...
	describe("tryFastFetch", () => {
		...
		test("should fetch and extract HTML content", async () => {
			global.fetch.mockResolvedValue({
				...
				arrayBuffer: () =>
					Promise.resolve(
						Buffer.from(
							"<html><body><p>Product info about SENSOR-X. This is a real product page with detailed specifications and pricing information.</p></body></html>"
						)
					)
			});
			...
			global.fetch.mockResolvedValue({
				ok: true,
				headers: {
					get: (h) => (h === "content-type" ? "text/html" : null)
				},
				arrayBuffer: () =>
					Promise.resolve(
						Buffer.from(
		...
		test("should return null for error pages", async () => {
			global.fetch.mockResolvedValue({
				...
				arrayBuffer: () =>
					Promise.resolve(Buffer.from("<html><body>404 Not Found</body></html>"))
			});
	...
	describe("isErrorPage - boundary cases", () => {
		...
		test("should detect PAGE NOT FOUND (uppercase)", () => {
			const text =
				"Sorry, PAGE NOT FOUND. Please try again later. This page does not exist in our system.";
