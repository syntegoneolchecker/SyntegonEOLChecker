...
describe("loginUser", () => {
    it("should reject users that do not exist", async () => {
        ...
        expect(
            await loginUser("doesNotExist@syntegon.com", "Password1"),
        ).toEqual({ success: false, message: "Invalid email or password" });
    });
    ...
    it("should reject users with locked accounts", async () => {
        ...
        expect(
            await loginUser("lockedAccount@syntegon.com", "Password1"),
        ).toEqual({
            success: false,
