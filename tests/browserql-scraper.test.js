...
describe('BrowserQL Scraper', () => {
    ...
    test('should throw if BROWSERQL_API_KEY is not set', async () => {
        ...
        await expect(scrapeWithBrowserQL('https://example.com'))
            .rejects.toThrow('BROWSERQL_API_KEY environment variable not set');
    });
    ...
    test('should return scraped content on success', async () => {
        global.fetch.mockResolvedValue({
            ...
            json: () => Promise.resolve({
                data: {
                    pageContent: {
                        value: JSON.stringify({ text: 'Product XYZ is discontinued', error: null })
                    }
                }
            })
        });
    ...
    test('should throw on HTTP error from BrowserQL API', async () => {
        ...
        await expect(scrapeWithBrowserQL('https://example.com'))
            .rejects.toThrow('BrowserQL API error: 429 - Token limit exceeded');
    });
    ...
    test('should throw on GraphQL errors in response', async () => {
        ...
        await expect(scrapeWithBrowserQL('https://example.com'))
            .rejects.toThrow('BrowserQL GraphQL errors');
    });
    ...
    test('should throw when no pageContent in response', async () => {
        ...
        await expect(scrapeWithBrowserQL('https://example.com'))
            .rejects.toThrow('BrowserQL returned no data');
    });
    ...
    test('should throw on evaluation error from page script', async () => {
        ...
        await expect(scrapeWithBrowserQL('https://example.com'))
            .rejects.toThrow('BrowserQL evaluation error: document.body is null');
    });
    ...
    test('should throw when evaluated content is empty', async () => {
        ...
        await expect(scrapeWithBrowserQL('https://example.com'))
            .rejects.toThrow('BrowserQL returned empty content');
    });
