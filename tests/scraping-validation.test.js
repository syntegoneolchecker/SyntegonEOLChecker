	...
	describe("isSafePublicUrl", () => {
		describe("valid public URLs", () => {
			...
			it("should accept URLs with paths", () => {
				expect(isSafePublicUrl("https://example.com/path/to/page")).toEqual({
					valid: true
				});
			});

			it("should accept URLs with query parameters", () => {
				expect(isSafePublicUrl("https://example.com?q=test&page=1")).toEqual({
					valid: true
				});
			});
			...
			it("should accept manufacturer URLs", () => {
				expect(
					isSafePublicUrl("https://www.keyence.co.jp/products/sensor/model-abc")
				).toEqual({ valid: true });
			});
		});
	...
	describe("isValidCallbackUrl", () => {
		...
		describe("with custom ALLOWED_ORIGINS", () => {
			it("should accept matching domain", () => {
				...
				const result = isValidCallbackUrl(
					"https://syntegoneolchecker.netlify.app/.netlify/functions/scraping-callback"
				);
				expect(result.valid).toBe(true);
			});
			...
			it("should accept subdomain of allowed domain", () => {
				...
				const result = isValidCallbackUrl(
					"https://syntegoneolchecker.netlify.app/callback"
				);
				expect(result.valid).toBe(true);
			});
