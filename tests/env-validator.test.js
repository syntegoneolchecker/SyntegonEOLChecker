...
describe("Environment Validator", () => {
	...
	describe("validateCommonEnvVars", () => {
		test("should throw if SITE_ID is missing", () => {
			...
			expect(() => envValidator.validateCommonEnvVars()).toThrow(
	...
	describe("validateBlobsToken", () => {
		test("should throw if both tokens are missing", () => {
			expect(() => envValidator.validateBlobsToken()).toThrow(
				"NETLIFY_BLOBS_TOKEN or NETLIFY_TOKEN"
			);
		});
	...
	describe("validateScrapingServiceUrl", () => {
		test("should throw if URL is missing", () => {
			expect(() => envValidator.validateScrapingServiceUrl()).toThrow(
				"SCRAPING_SERVICE_URL environment variable is required"
			);
		});
		...
		test("should throw if URL format is invalid", () => {
			...
			expect(() => envValidator.validateScrapingServiceUrl()).toThrow(
				"Invalid SCRAPING_SERVICE_URL format"
			);
		});
