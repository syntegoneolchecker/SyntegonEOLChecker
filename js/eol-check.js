...
async function wakeRenderService(checkButton) {
	...
	if (!isRenderServiceHealthy()) {
		...
		showStatus(
			`Error: Render Scraping Service is not available (${renderStatusText}). Please reload the page and try again.`,
			"error"
		);
		return false;
	}
...
export function disableAllCheckEOLButtons() {
	...
	console.log(
		"Check EOL buttons, manual trigger button and delete toggle disabled (manual check in progress)"
	);
}
...
export function enableAllCheckEOLButtons() {
	...
	console.log(
		"Check EOL buttons, manual trigger button and delete toggle re-enabled (manual check complete)"
	);
}
...
async function triggerFetchUrl(jobId, firstUrl, attempts) {
	...
	for (let retry = 0; retry <= maxRetries; retry++) {
		try {
			...
			console.warn(
				`⚠️  fetch-url returned ${response.status} (retry ${retry}/${maxRetries})`
			);
		} catch (err) {
			if (err.name === "TimeoutError" || err.name === "AbortError") {
				console.log(
					`⏱️  fetch-url request timed out - function likely still running server-side, NOT retrying`
				);
				return;
			}

			console.error(
				`Failed to trigger fetch-url (retry ${retry}/${maxRetries}): ${err.message}`
			);
		}
...
async function triggerAnalyzeJob(jobId, attempts) {
	...
	for (let retry = 0; retry <= maxRetries; retry++) {
		try {
			...
			console.warn(
				`⚠️  analyze-job returned ${response.status} (retry ${retry}/${maxRetries})`
			);
		} catch (err) {
			if (err.name === "TimeoutError" || err.name === "AbortError") {
				console.log(
					`⏱️  analyze-job request timed out - function likely still running server-side, NOT retrying`
				);
				return;
			}

			console.error(
				`Failed to trigger analyze-job (retry ${retry}/${maxRetries}): ${err.message}`
			);
		}
...
export async function pollJobStatus(jobId, manufacturer, model, checkButton) {
	...
	while (attempts < maxAttempts) {
		...
		try {
			...
			console.log(
				`Trigger flags: fetchTriggered=${fetchTriggered}, analyzeTriggered=${analyzeTriggered}`
			);
